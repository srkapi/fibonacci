...
public class LoggingMiddleware implements Middleware {
  ...
  public <R> R handle(Message<R> message, NextMiddlewareFunction<Message<R>, R> next)
    ...
    log.info(
        String.format(
            "Received %s (%s), the %s has been dispatched to %s bus",
    ...
    try {
      ...
      log.info(
          String.format(
              "The %s %s (%s) has been handled successfully with result: %s",
              getMessageType(message),
              message.getClass().getName(),
              message.toString(),
              serializedResult));
      ...
      log.error(
          String.format(
              "Failed to handle %s (%s)", message.getClass().getName(), message.toString()),
          ex);
      throw ex;
    }
  }
